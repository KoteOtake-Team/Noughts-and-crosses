...
def start_game(id=-1):
    ...
    СОЗДАТЬ ПАРТИЮ
    Принимает:
        id (int, необязательный, равен -1 по умолчанию) - идентификатор игрока, с которым хотелось бы сыграть
    Возвращает:
    ...
    value = nc_net.transfer() if id == -1 else nc_net.transfer(id=id)  # обмен данными с сервером
